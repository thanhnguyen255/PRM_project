...
class InClassDetailScreen extends StatefulWidget {
  final int activityId;
  const InClassDetailScreen({super.key, required this.activityId});

  @override
  State<InClassDetailScreen> createState() => _InClassDetailScreenState();
}

class _InClassDetailScreenState extends State<InClassDetailScreen> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    if (activity != null) {
      ...
      return Scaffold(
        ...
        appBar: AppBar(title: const Text('Chi tiết hoạt động In-Class')),
        body: vm.isLoading
            ? const LoadingWidget()
                      ...
                      children: [
                        Container(
                          ...
                          child: Text(
                            ...
                            style: const TextStyle(color: inClassColor, fontWeight: FontWeight.bold),
                          ),
                        ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: submissionStatus == 'Approved'
                            ? null
                            : () {
                                Navigator.pushNamed(context, '/submit-in-class-evidence', arguments: widget.activityId);
                              },
                      ...
                      SizedBox(
                        ...
                        child: OutlinedButton.icon(
                          ...
                          style: OutlinedButton.styleFrom(
                            foregroundColor: inClassColor,
                            padding: const EdgeInsets.symmetric(vertical: 16),
                            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                            side: const BorderSide(color: inClassColor),
                            textStyle: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold),
                          ),
                        ),
                      ),
